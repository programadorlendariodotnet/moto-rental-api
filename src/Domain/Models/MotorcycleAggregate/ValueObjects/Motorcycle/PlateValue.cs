...
public class PlateValue(string value)
{
    ...
    public static Result<PlateValue> Create(string reasonValue, string entity)
    {
        var isValid = Validate(reasonValue, entity);
        return isValid.IsFailed ? isValid : new PlateValue(reasonValue);
    }
}